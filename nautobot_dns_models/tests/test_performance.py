...
class IPAssignmentPerformanceTestCase(TestCase):
    ...
    def test_ip_assignment_performance_absolute_baseline(self):
        ...
        try:
            ...
            print(f"Assigned 128 IPs to 128 interfaces")
    ...
    def test_single_ip_assignment_sql_analysis_with_rules(self):
        ...
        a_record_count = ARecordModel.objects.filter(
            name=f"{interface.name}.{interface.device.name}"
        ).count()
        tracking_count = DNSRuleRecord.objects.filter(
            object_id=str(interface.pk)
        ).count()

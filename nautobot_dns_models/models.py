...
class DNSRule(DNSModel):
    ...
    class Meta:
        ...
        constraints = [
            models.UniqueConstraint(fields=["content_type", "record_type"], name="unique_content_type_record_type")
        ]

    # Basic fields
    name = models.CharField(max_length=100, unique=True, help_text="Name of the rule")
    description = models.TextField(blank=True, help_text="Description of the rule's purpose")
    ...
    priority = models.PositiveIntegerField(default=1000, help_text="Priority of this rule (lower numbers run first)")
    ...
    name_template = models.TextField(help_text="Jinja2 template for generating the DNS record name")
            ...
            if template:
                try:
                    ...
                    raise ValidationError(
                        {"template": f"[model] Invalid template syntax in {template_field}: {str(e)}"}
                    )

        # Validate MX preference is set if record type is MX
        if self.record_type == "MX" and self.mx_preference is None:
            raise ValidationError({"mx_preference": "Preference is required for MX records"})

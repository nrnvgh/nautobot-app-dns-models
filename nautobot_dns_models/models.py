...
def dns_wire_name(name):
    """Return `name` encoded as an absolute DNS name in wire format, per RFC 1035 §3.1.

    Lowercased first, so that a zone entered as `Example.com` encodes identically to `example.com`;
    RFC 4034 §6.2 makes lowercase the canonical form, and DNS name comparison ignores case anyway.
    """
    wire = b""
    for label in name.rstrip(".").lower().split("."):
        encoded = label.encode("ascii") if label.isascii() else b"xn--" + label.encode("punycode")
        wire += len(encoded).to_bytes(1, "big") + encoded
    return wire + b"\x00"


def catalog_member_label(zone_name):
    """Return the catalog zone label identifying a member zone named `zone_name`.

    RFC 9432 §4.1 lets a producer pick any unique label, so this follows BIND's documented
    convention of the SHA-1 of the member name in wire format, as generated by its
    `contrib/scripts/catzhash.py`. The digest is 40 hex characters, well inside a DNS label.
    """
    return hashlib.sha1(dns_wire_name(zone_name)).hexdigest()  # noqa: S324
...
class CatalogZoneMember(PrimaryModel):
    ...
    member_label = models.CharField(
        ...
        help_text=(
            ...
            "Generated from the member zone name if left blank, and fixed thereafter."
        ),
    ...
    def _ensure_member_label(self):
        ...
        if not self.member_label and self.member_zone_id:
            self.member_label = catalog_member_label(self.member_zone.name)

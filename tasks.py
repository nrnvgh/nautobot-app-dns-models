...
def _ensure_creds_env_file(context):
    ...
    if not os.path.exists(os.path.join(context.nautobot_dns_models.compose_dir, "creds.env")):
        ...
        shutil.copy(
            os.path.join(context.nautobot_dns_models.compose_dir, "creds.example.env"),
            os.path.join(context.nautobot_dns_models.compose_dir, "creds.env"),
        )
...
def djhtml(context, check=False):
    ...
    command = "djhtml -t 4 nautobot_dns_models/templates/"

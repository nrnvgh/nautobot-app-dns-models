...
A zone's REST API representation carries a read-only `catalog` field naming the catalog it is enrolled in, or `null` when it is not enrolled. Memberships themselves are created, moved, and removed through the Catalog Zone Member endpoint.
